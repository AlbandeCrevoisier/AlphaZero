...
class AlphaZeroConfig(object):
    ...
    def __init__(self, goban_size=19, num_filters=256, num_residuals=19):
        ...
        self.momentum = 0.9
        self.learning_rate_schedule = {0: 2e-2}
...
class Game(object):
    ...
    def store_search_statistics(self, root):
        ...
        self.child_visits.append([
            ...
            else 0 for a in range(self.num_actions)])
...
class Network(object):
    ...
    def __init__(self, config: AlphaZeroConfig):
        ...
        # First block
        x = Conv2D(nfilters, 3, 1, 'same', kernel_regularizer=l2(w_d))(input)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)

        # Residual blocks
        for _ in range(nresiduals):
            tmp = Conv2D(nfilters, 3, 1, 'same', kernel_regularizer=l2(w_d))(x)
        ...
        p = Conv2D(2, 1, 1, 'same', kernel_regularizer=l2(w_d))(x)
        ...
        # Value head
        v = Conv2D(1, 1, 1, 'same', kernel_regularizer=l2(w_d))(x)
